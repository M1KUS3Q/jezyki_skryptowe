...
type SeriesCache = Path | dict[StationCode, TimeSeries]


class Measurements:
    def __init__(self, path: str | os.PathLike[str] | Path) -> None:
        self.path: Path = Path(path)
        self.paths_by_quantity: dict[Quantity, list[SeriesCache]] = {}
        self.total_series_count: int = -1

        pattern: str = r"\d{4}_([^_]+)_(?:1g|24g)\.csv"
    ...
    def _load_file(self, path: Path) -> dict[StationCode, TimeSeries]:
        series_by_station: dict[StationCode, TimeSeries] = {}
        ...
        with open(path, mode="r", encoding="utf-8") as f:
            ...
            for line in f:
                ...
                row_name: str = cols[0].strip()
                headers[row_name] = cols

                if row_name == "Kod stanowiska":
                    break

            indicator_row: list[str] = headers.get("Wskaźnik", [])
            avg_time_row: list[str] = headers.get("Czas uśredniania", [])
            unit_row: list[str] = headers.get("Jednostka", [])
            station_code_row: list[str] = headers.get("Kod stacji", [])

            stations_count: int = len(station_code_row) - 1
            ...
            for line in f:
                ...
                for i in range(1, len(cols)):
                    ...
                    val_str: str = cols[i].strip()
                    val: float | None = float(val_str.replace(",", ".")) if val_str else None
